...
export default function DoctorSearch() {
  ...
  const [filters, setFilters] = useState({
    name: "", specialty: "", date: "", startTime: "", minPrice: "", maxPrice: "", minRating: ""
  });

  useEffect(() => {
    const initPage = async () => {
      try {
        const [specsData, docsData] = await Promise.all([
        ...
        toast.error("Error loading doctor search data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const clearFilters = async () => {
    const emptyFilters = { name: "", specialty: "", date: "", startTime: "", minPrice: "", maxPrice: "", minRating: "" };
    setFilters(emptyFilters);
    setLoading(true);
    try {
      const data = await searchDoctors(emptyFilters);
      setDoctors(data.doctors || data || []);
    } catch (error) {
      console.error(error);
    } finally {
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-fit lg:sticky lg:top-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={clearFilters} className="text-sm text-blue-600 font-medium hover:underline">Clear</button>
          </div>

          <form onSubmit={handleSearchSubmit} className="space-y-5">
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Doctor Name</label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Start Time (24H)</label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Price Range ($)</label>
              <div className="grid grid-cols-2 gap-2">
                <input type="number" name="minPrice" value={filters.minPrice} onChange={handleInputChange} placeholder="Min" className="w-full bg-gray-50 border border-gray-200 px-3 py-2 rounded-xl outline-none focus:border-blue-500 text-sm" />
                <input type="number" name="maxPrice" value={filters.maxPrice} onChange={handleInputChange} placeholder="Max" className="w-full bg-gray-50 border border-gray-200 px-3 py-2 rounded-xl outline-none focus:border-blue-500 text-sm" />
              </div>
            </div>

            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Min Rating</label>
        ...
        <div className="lg:col-span-3">
          {loading ? (
            ...
            <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300">
              <p className="text-gray-500 text-lg">No doctors matched your specific criteria.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {doctors.map((doc) => (
                <div key={doc.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-xl overflow-hidden">
                          {doc.user?.profile_picture ? <img src={doc.user.profile_picture} className="w-full h-full object-cover" alt="avatar" /> : (doc.user?.name?.charAt(0) || "D")}
                        </div>
                        <div>
                    ...
                    <div className="mb-4">
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Available Slots</p>
                      <div className="flex flex-wrap gap-2">
                        {/* Render slots dynamically if backend includes them, otherwise show a prompt */}
                        {doc.slots && doc.slots.length > 0 ? (
                          doc.slots.slice(0, 4).map(slot => (
                            <span key={slot.id} className="bg-emerald-50 border border-emerald-100 text-emerald-700 text-xs px-2 py-1 rounded-md font-medium">
                              {new Date(slot.date).toLocaleDateString(undefined, {month:'short', day:'numeric'})} • {slot.start_time?.substring(0, 5)}
                            </span>
                          ))
                        ) : (
                          <span className="text-gray-400 text-xs italic">View profile to see full availability schedule.</span>
                        )}
                        {doc.slots && doc.slots.length > 4 && <span className="text-gray-400 text-xs px-1 py-1">+{doc.slots.length - 4} more</span>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                    <div>
                      <p className="text-gray-400 text-[10px] font-bold uppercase tracking-wide">Fee</p>
                      <p className="text-xl font-black text-gray-800">${parseFloat(doc.consultation_fee || 0).toFixed(2)}</p>
                    </div>
                    <button 
                      onClick={() => navigate(`/doctor/${doc.id}`)}
                      className="bg-blue-600 text-white hover:bg-blue-700 px-5 py-2 rounded-xl font-bold text-sm transition"
                    >
                      View Profile
                    </button>
                  </div>
